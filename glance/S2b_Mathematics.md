...
### Fluency Score — S9b/log(RT2)

La matrice 2x2 Speed-Accuracy est informative mais binaire. La **Fluency Score** est une métrique continue qui fusionne précision et vitesse :

```
Fluency(i) = {
    1 / log(RT₂(i))    si S9b(i) = 1
    0                   si S9b(i) = 0
}
```

où `RT₂(i)` est plafonné à min(RT₂, 100ms) pour éviter log(0).

**Propriétés** :
- Fluency > 0 seulement si la réponse est correcte (pas de récompense pour deviner vite et faux)
- Fluency diminue logarithmiquement avec le temps — un passage de 1s à 2s est plus pénalisant qu'un passage de 8s à 9s
- Plage typique : ~0.10 (correct à 20s) à ~0.22 (correct à 100ms)

**Métrique agrégée** :
```
Avg_Fluency(GA) = Σ Fluency(i) / N
```

**Avantage sur le binaire S9b** : Deux GAs peuvent avoir le même Taux_S9b = 80%, mais si l'un a un Avg_Fluency de 0.18 (réponses rapides) et l'autre 0.12 (réponses lentes), le premier encode *mieux* la hiérarchie — l'information est accessible sans effort cognitif. La Fluency Score capture cette différence que le binaire masque.

**Justification** : L'inverse du logarithme du temps de réaction est utilisée en psychométrie comme mesure de l'efficience cognitive (Thorne, 2006, *Cognitive Processing*). La transformation logarithmique est appropriée car les RT suivent une distribution log-normale (Luce, 1986).
...
### Fluency Delta (complément continu)

En parallèle du McNemar binaire, le **delta de fluency** compare les moyennes de Fluency(i) = S9b/log(RT2) entre les groupes :

```
Avg_Fluency(VEC) - Avg_Fluency(contrôle)
```

Ce delta capture non seulement *si* plus de participants ont répondu correctement, mais aussi *à quelle vitesse*. Un delta positif indique que le VEC produit des réponses à la fois plus justes et plus rapides. Pour les données continues appariées, le test de **Wilcoxon signed-rank** remplace le McNemar (qui est limité au binaire).
...
**Upgrade Phase 4 — GLMM plutôt que McNemar** :

Quand le volume de données le permet (N=500+), la comparaison A/B binaire par McNemar est remplacée par un **Generalized Linear Mixed Model (GLMM)** avec la **Fluency Score** comme variable dépendante continue :

```
Fluency(i,j) = β₀ + β₁·ga_version + β₂·clinical_domain + ... + u_j + ε_ij
```

où `u_j ~ N(0, σ²_u)` est l'effet aléatoire par participant (intercept aléatoire), et `ε_ij` est l'erreur résiduelle.

**Pourquoi GLMM > McNemar :**

1. **Variable continue** : Fluency = S9b/log(RT2) encode à la fois l'exactitude ET la vitesse. McNemar ne voit que correct/incorrect et perd toute l'information temporelle.
2. **Effets aléatoires** : Le GLMM modélise explicitement la variabilité inter-participants (certains sont systématiquement plus rapides/lents), ce que McNemar ignore.
3. **Covariables simultanées** : Profil, résolution d'écran, dwell time, modalité Q1 — tout entre dans un seul modèle au lieu de tests séparés.
4. **Puissance** : La Fluency continue donne plus de puissance statistique que le binaire S9b, réduisant le N requis d'environ 30-40% pour le même effet.

Le McNemar reste le test principal en Phase 3 (N=30-80) car il ne nécessite pas d'hypothèse distributionnelle et est robuste aux petits échantillons. Le GLMM prend le relais quand les données justifient un modèle plus riche.

**Justification** : Les GLMMs sont recommandés par Baayen et al. (2008, *Journal of Memory and Language*) pour les données psycholinguistiques avec observations répétées par participant. Le passage de McNemar (binaire apparié) au GLMM (continu avec effets aléatoires) suit l'évolution standard de la méthodologie en psychologie expérimentale (Jaeger, 2008, *Journal of Memory and Language*).
...
```
Score_GLANCE = (0.2 × S9a) + (0.5 × S9b) + (0.3 × S9c)     ∈ [0, 1]
```
...
**Affichage** :
- Score_GLANCE ≥ 0.70 → "Décodage réussi"
- Score_GLANCE < 0.70 → "Le design n'a pas survécu au transfert"
...
**Valeur pour le paper :** La distribution des motifs de rejet par profil (quadrant 2D) est une donnée originale. Si les cliniciens rejettent pour "trop de texte" et les data scientists pour "aspect amateur", c'est une preuve que le design doit cibler un profil — et que le profilage GLANCE est nécessaire.
...
**Implication pour le feed simulé :** Si le flux GLANCE est composé de 8 posts, le mix réaliste est :
...
| Score_GLANCE | 0.2·S9a + 0.5·S9b + 0.3·S9c | Score composite | ≥0.70 |
