"""
Parametric GA Compositor V15 — 2-Panel Layout (Aurore's design alignment)

V14 had 3 side-by-side zones (margin_left, bronchus, margin_right).
...
Reads config/layout_v15.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v15.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v15.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
def draw_evidence_bars(dwg, layout, palette, W, H):
    """V15: Evidence bars in the bottom-right of the top panel."""
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
...
# ===================================================================
# V15: BOTTOM PANEL — mechanism flow (WHAT -> HOW -> RESULT)
# ===================================================================
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v15.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v15_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v15_delivery.png")
