"""
Parametric GA Compositor V11 — "La Bronche Vivante" + Aurore's 3-column design

Inherits V10.5 bronchial cross-section with all organic density features.
New in V11 (from Aurore's design):
  1. Zone headers — "Early life window" (left), "Preclinical data" (center), "Clinical data" (right)
  2. Product pills — 4 colored rounded pills at top center
  3. Mechanism boxes — 4 labeled boxes below bronchus (center zone)
  4. Disease cascade — vertical flow below sick child (left margin)
  5. Bigger evidence bars — product name + RCT count inside the bar
  6. Increased whitespace and font sizes for clarity

Reads config/layout_v11.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v11.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v11.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
# ===================================================================
# V11 NEW: ZONE HEADERS
# ===================================================================

def draw_zone_headers(dwg, layout, palette, content, W, H):
    """Draw 3 zone header pills: Early life window (red), Preclinical data (gray), Clinical data (green)."""
    ...
    # Left zone header — red pill over margin_left
    ...
    dwg.add(dwg.rect((pill_x, y), (pill_w, pill_h),
                     ...
                     ry=left_cfg["pill_rx"], opacity=0.90))
    dwg.add(dwg.text(left_text,
                     insert=(pill_x + pill_w // 2, y + pill_h * 0.70),
                     font_size=fs, fill=left_cfg["text_color"],
                     font_family=font_family, font_weight="bold",
                     text_anchor="middle"))

    # Center zone header — gray pill over bronchus zone
    ...
    dwg.add(dwg.rect((pill_x_c, y), (pill_w_c, pill_h),
                     ...
                     ry=center_cfg["pill_rx"], opacity=0.90))
    dwg.add(dwg.text(center_text,
                     insert=(pill_x_c + pill_w_c // 2, y + pill_h * 0.70),
                     font_size=fs, fill=center_cfg["text_color"],
                     font_family=font_family, font_weight="bold",
                     text_anchor="middle"))

    # Right zone header — green pill over margin_right
    ...
    dwg.add(dwg.rect((pill_x_r, y), (pill_w_r, pill_h),
                     ...
                     ry=right_cfg["pill_rx"], opacity=0.90))
    dwg.add(dwg.text(right_text,
                     insert=(pill_x_r + pill_w_r // 2, y + pill_h * 0.70),
...
# ===================================================================
# V11 NEW: PRODUCT PILLS (top center)
# ===================================================================

def draw_product_pills(dwg, layout, palette, content, W, H):
    """Draw 4 colored product pills in a horizontal row above the bronchus."""
...
# ===================================================================
# V11 NEW: MECHANISM BOXES (below bronchus, center zone)
# ===================================================================

def draw_mechanism_boxes(dwg, layout, palette, content, W, H):
    """Draw 4 mechanism boxes stacked vertically below the bronchus.
    Each box: rounded rect, colored left border, bold title + smaller subtitle."""
    ...
    subtitle_fs = mb["subtitle_font_size"]
    bg_color = mb["bg_color"]
    bg_opacity = mb["bg_opacity"]
    border_color = mb["border_color"]

    box_width = bz_w - 60  # some padding from zone edges
    box_x = bz_x + 30

    for i, box_data in enumerate(boxes):
        box_y = y + i * (box_h + box_gap)

        # Get product color for left border
        ...
        dwg.add(dwg.rect((box_x, box_y + 4), (border_w, box_h - 8),
                         fill=left_border_color, rx=3, ry=3, opacity=0.90))

        # Title (bold)
        title_x = box_x + border_w + 18
        title_y = box_y + box_h * 0.42
        ...
        subtitle_y = box_y + box_h * 0.78
        dwg.add(dwg.text(box_data["subtitle"],
                         insert=(title_x, subtitle_y),
...
# ===================================================================
# V11 NEW: DISEASE CASCADE (left margin, below sick child)
# ===================================================================

def draw_disease_cascade(dwg, layout, palette, content, W, H):
    """Draw vertical cascade: Inflammation -> Epithelial injury -> ... -> Risk of Asthma
    with downward arrows and red-tinted pill boxes."""
    ...
    bg_color = dc_layout["bg_color"]
    text_color = dc_layout["text_color"]
    border_color = dc_layout["border_color"]
    ...
    for i, step_text in enumerate(steps):
        ...
        # Gradient intensity: gets redder as cascade progresses
        ...
        dwg.add(dwg.rect((step_x, step_y), (step_w, step_h),
                         fill=step_bg, stroke=step_border, stroke_width=1.5,
        ...
        if i < len(steps) - 1:
            ...
            dwg.add(dwg.line((arrow_x, arrow_top), (arrow_x, arrow_bottom),
                             stroke=arrow_color, stroke_width=2.5, opacity=0.65))
            # Arrowhead
            dwg.add(dwg.polygon([
                (arrow_x - 5, arrow_bottom - 2),
                (arrow_x, arrow_bottom + 4),
                (arrow_x + 5, arrow_bottom - 2),
            ], fill=arrow_color, opacity=0.65))


# ===================================================================
# DRAWING: BACKGROUND + GRADIENT (from V10)
# ===================================================================
...
# ===================================================================
# DRAWING: BRONCHUS FRAME + BAND SEPARATORS (from V10)
# ===================================================================
...
def draw_band_labels(dwg, layout, palette, W, H):
    ...
    font_family = layout["font"]["family"]
    label_color = "#6B7280"
    label_font_size = 22  # slightly bigger for V11
...
# ===================================================================
# ZONE ORIENTATION TEXT (from V10)
# ===================================================================
...
def draw_zone_orientation(dwg, layout, palette, W, H):
    ...
    dwg.add(dwg.rect((path_x - 4, path_y - 24), (185, 30),
    ...
    prot_x = bx + bw - 20
    dwg.add(dwg.rect((prot_x - 145, by - 36), (150, 30),
                     fill="white", opacity=0.6, rx=3))
...
# ===================================================================
# DRAWING: BAND 1 — LUMEN (from V10.5)
# ===================================================================
...
def draw_lumen(dwg, layout, palette, W, H):
    ...
    # Extra small virus particles scattered on sick side
    ...
    # Convergence lines from actual product locations
    bands = get_band_rects(layout, W, H)
...
# ===================================================================
# DRAWING: BAND 2 — EPITHELIUM (from V10.5)
# ===================================================================
...
def draw_epithelium(dwg, layout, palette, W, H):
    ...
    # Stipple texture behind epithelium
    ...
    for i in range(n_cells):
        ...
        if i in gap_indices:
            ...
            # Nucleus hints inside epithelial cells
            ...
            # Tight junction zigzags (healthy side)
...
# ===================================================================
# DRAWING: BAND 3 — LAMINA PROPRIA (from V10.5)
# ===================================================================
...
def draw_lamina_propria(dwg, layout, palette, W, H):
    ...
    # Cross-hatch texture (connective tissue grain)
    ...
    # T-helper cells scattered in upper lamina
...
# ===================================================================
# DRAWING: BAND 4 — MUSCLE LISSE (from V10.5)
# ===================================================================
...
# ===================================================================
# DRAWING: VICIOUS CYCLE (from V10)
# ===================================================================
...
# ===================================================================
# DRAWING: CYCLE-BREAK VISUAL (from V10)
# ===================================================================
...
# ===================================================================
# V11: BIGGER EVIDENCE BARS (product name + RCT count inside bar)
# ===================================================================

def draw_evidence_bars(dwg, layout, palette, W, H):
    """V11: Bigger evidence bars with product name AND RCT count inside the bar."""
    mr_x, _, mr_w, _ = get_zone_rect(layout, "margin_right", W, H)
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
    ...
    for item in ev["items"]:
        ...
        dwg.add(dwg.rect((bar_x, y), (bar_w, bar_h),
                         fill=color, rx=6, ry=6, opacity=bar_opacity))

        # Product name (left-aligned inside bar, bold)
        ...
        if bar_w > 160:
            ...
            # Just product name inside, label outside
            dwg.add(dwg.text(product_name,
            ...
            # Small bar: product name inside, label outside
            dwg.add(dwg.text(product_name,
...
# ===================================================================
# DRAWING: CRL1505 RELAY ARC (from V10)
# ===================================================================
...
# ===================================================================
# DRAWING: PRODUCT LEGEND (below everything)
# ===================================================================
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v11.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v11_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v11_delivery.png")
    ...
    # 11. Disease cascade (V11 new — below sick child)
