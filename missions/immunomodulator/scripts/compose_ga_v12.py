"""
Parametric GA Compositor V12 — GLANCE feedback iteration

Changes from V11:
  1. Word count CUT: 65 -> ~28 (mechanism boxes = single keywords, cascade simplified)
  2. Saliency BOOST: children 1.5x bigger, evidence bars wider/taller, stronger gradient
  3. Readability at 50%: min font 36pt, all headers/labels scaled up
  4. Accessibility: hatching patterns on product-colored elements for non-color differentiation

Reads config/layout_v12.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v12.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v12.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
# ===================================================================
# V12: ZONE HEADERS (bigger)
# ===================================================================

def draw_zone_headers(dwg, layout, palette, content, W, H):
    """Draw 3 zone header pills — bigger for V12 readability."""
...
# ===================================================================
# V12: PRODUCT PILLS (bigger)
# ===================================================================

def draw_product_pills(dwg, layout, palette, content, W, H):
    """Draw 4 colored product pills — bigger for V12."""
...
# ===================================================================
# V12: MECHANISM BOXES (single keyword, no subtitle)
# ===================================================================

def draw_mechanism_boxes(dwg, layout, palette, content, W, H):
    """Draw 4 mechanism boxes — V12: single keyword per box, bigger font."""
    ...
    for i, box_data in enumerate(boxes):
        ...
        # Colored left border strip — thicker for V12
        ...
        # Title only (V12: no subtitle)
        title_x = box_x + border_w + 22
...
# ===================================================================
# V12: DISEASE CASCADE (simplified text, bigger font)
# ===================================================================
    ...
    for bname, (bx2, by2, bw2, bh2) in bands.items():
        if bname in band_gradient_triplets:
            ...
            dwg.add(dwg.rect((bx2, by2), (bw2, bh2),
                             fill=f"url(#{grad_id})", opacity=0.50))  # slightly more saturated
        elif bname == "muscle":
            dwg.add(dwg.rect((bx2, by2), (bw2, bh2),
                             fill=palette["bands"]["lamina_bg"],
                             opacity=0.3))

    # Separator lines — thicker for V12
...
# ===================================================================
# BAND LABELS — BIGGER for V12
# ===================================================================

def draw_band_labels(dwg, layout, palette, W, H):
    bands = get_band_rects(layout, W, H)
    font_family = layout["font"]["family"]
    label_color = "#4B5563"    # darker for contrast
    label_font_size = 28       # was 22 — bigger for 50% zoom
...
# ===================================================================
# V12: BIGGER EVIDENCE BARS with accessibility patterns
# ===================================================================

def draw_evidence_bars(dwg, layout, palette, W, H):
    """V12: Bigger evidence bars with product name + RCT count + accessibility patterns."""
    mr_x, _, mr_w, _ = get_zone_rect(layout, "margin_right", W, H)
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v12.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v12_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v12_delivery.png")
    ...
    # 10. Child silhouettes — 1.5x BIGGER
