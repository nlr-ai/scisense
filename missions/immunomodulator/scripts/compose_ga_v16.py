...
Reads config/layout_v16.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v16.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v16.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
def draw_evidence_bars(dwg, layout, palette, W, H):
    """V16: Evidence bars — wider, with outcome labels per product."""
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v16.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v16_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v16_delivery.png")
