"""
GA Wireframe v4 — Converged Design (3 axes merged)

Major changes from v3:
- Narrative spine: 3 CIRCULAR bronchial cross-sections (sick → transitioning → healthy)
  replaces horizontal tube
- Product encoding: products ACT ON the bronchial cross-section (shield, bricks, helix, bridge)
  replaces separate icons above tube
- Evidence communication: translational staircase (Laboratory → Bedside)
  replaces horizontal fill bars
- Child pictograms in Zones 1 and 3
- Vicious cycle AROUND Zone 1 cross-section (not below tube)
- Opacity gradient on Zone 2 product elements (primes for Zone 3 evidence)
- Zone backgrounds: pale red / pale warm / pale green
"""
...
svg_path = os.path.join(OUT_DIR, "wireframe_GA_v4.svg")
...
def draw_child(dwg, cx, cy, color, healthy=False):
    """Simple child stick figure. head circle + body + arms + legs."""
    head_r = 18
    # Head
    ...
    if healthy:
        ...
        dwg.add(dwg.line((cx, arm_y), (cx - arm_span, arm_y - 20),
                         stroke=color, stroke_width=3))
        dwg.add(dwg.line((cx, arm_y), (cx + arm_span, arm_y - 20),
                         stroke=color, stroke_width=3))
    else:
        # Arms down (tired)
        dwg.add(dwg.line((cx, arm_y), (cx - arm_span, arm_y + 18),
                         stroke=color, stroke_width=3))
        dwg.add(dwg.line((cx, arm_y), (cx + arm_span, arm_y + 18),
                         stroke=color, stroke_width=3))
    # Legs
    leg_span = 20
    dwg.add(dwg.line((cx, body_bottom), (cx - leg_span, body_bottom + 35),
                     stroke=color, stroke_width=3))
    dwg.add(dwg.line((cx, body_bottom), (cx + leg_span, body_bottom + 35),
                     stroke=color, stroke_width=3))
...
z2_cy = H * 0.50                  # Cross-section center
bronch2_r = 220                   # Larger cross-section for Zone 2
...
#    Teal bricks patching the epithelium
brick_positions = []
for angle_deg in [340, 0, 20]:
    angle = math.radians(angle_deg)
...
# MV130 label (inside lumen area, upper left)
mv130_lx = z2_cx - rings2["lumen_r"] * 0.1
mv130_ly = z2_cy - rings2["lumen_r"] * 0.25
mv130_style = f"font-family:Arial;font-size:34px;fill:{C_MV130};text-anchor:middle;font-weight:bold"
...
dwg.add(dwg.path(d=bridge_d, fill="none",
                 stroke=C_CRL1505, stroke_width=5, opacity=0.3))

# Bridge pillars
for px in [bridge_cx - bridge_width/2, bridge_cx + bridge_width/2]:
    dwg.add(dwg.line((px, bridge_top_y), (px, bridge_top_y + 35),
                     stroke=C_CRL1505, stroke_width=3, opacity=0.3))
...
# Gut-lung axis: small icon indicators instead of text labels (saves word budget)
# Small circle at each pillar base to suggest organs
dwg.add(dwg.circle(center=(bridge_cx - bridge_width/2, bridge_top_y + 45),
                   r=8, fill=C_CRL1505, opacity=0.15,
...
# ── Opacity gradient visual cue: product action elements ──
# OM-85 at full opacity (done above, stroke_width=14, opacity=1.0)
# PMBL at 0.7 (bricks drawn at opacity=0.5 — acceptable)
# MV130 at 0.5 (helix drawn at opacity=0.08 fill + 0.5 stroke — via path opacity)
# CRL1505 at 0.3 (bridge drawn at opacity=0.3 — done above)
...
full_png = os.path.join(OUT_DIR, "wireframe_GA_v4_full.png")
delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v4_delivery.png")
