"""
Parametric GA Compositor V13 — Restore clarity + hierarchy

V12 REGRESSED: mechanism box labels cut too aggressively ("Barrier ^" etc. are cryptic).
Lesson: word budget is SOFT (~35-40 OK). Hierarchy > parsimony.

Changes from V12:
  1. Mechanism boxes: restore SHORT but informative phrases (+4 words total)
     "Barrier ^" -> "Epithelial Barrier ^", etc.
  2. Mechanism boxes: add 40x40 geometric ICONS left of text
     barrier=horizontal lines, innate=starburst, adaptive=Y-shape, inflammation=down-arrow
  3. Evidence bars: MORE prominent (moved up, taller, title underline)
  4. Zone headers: MORE prominent (bigger font, taller pills)
  5. Band labels: SMALLER (expert detail, not primary hierarchy)
  6. Connecting arrow from mechanism boxes to evidence bars (flow visualization)
  7. Keep ALL V12 visual gains (big children, big bars, 36pt min, accessibility, gradients)

Reads config/layout_v13.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v13.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v13.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
# ===================================================================
# V13: ZONE HEADERS (more prominent)
# ===================================================================

def draw_zone_headers(dwg, layout, palette, content, W, H):
    """Draw 3 zone header pills — V13: more prominent."""
...
# ===================================================================
# V13: MECHANISM BOXES — restored labels + icons
# ===================================================================
...
# ===================================================================
# V13: DISEASE CASCADE (same as V12)
# ===================================================================
...
# ===================================================================
# BAND LABELS — V13: SMALLER (expert detail, recedes)
# ===================================================================
...
def draw_band_labels(dwg, layout, palette, W, H):
    ...
    # V13: use layout config for smaller band labels
    bl_cfg = layout.get("band_labels", {})
    label_font_size = bl_cfg.get("font_size", 22)
    label_color = bl_cfg.get("color", "#6B7280")
...
# ===================================================================
# V13: BIGGER EVIDENCE BARS + title underline + accessibility
# ===================================================================

def draw_evidence_bars(dwg, layout, palette, W, H):
    """V13: More prominent evidence bars with title underline."""
    mr_x, _, mr_w, _ = get_zone_rect(layout, "margin_right", W, H)
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
    ...
    # V13: Title underline for emphasis
...
def draw_legend(dwg, layout, palette, content, W, H):
    ...
    total_w = len(products) * (leg["square_size"] + leg["gap"] + 90)
    start_x = (W - total_w) // 2

    for i, (name, color, key) in enumerate(products):
        x = start_x + i * (leg["square_size"] + leg["gap"] + 90)
        # Solid color square
        ...
        if pat_id:
            dwg.add(dwg.rect((x, y), (leg["square_size"], leg["square_size"]),
                             fill=f"url(#{pat_id})", rx=4, opacity=0.5))
        dwg.add(dwg.text(name,
                         insert=(x + leg["square_size"] + 10, y + leg["square_size"] * 0.78),
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v13.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v13_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v13_delivery.png")
