"""
Parametric GA Compositor V18 — Multi-Channel Evidence

V17 only used bars (length channel). V18 adds 6 channels per P1 claim:
  1. length      → bar width proportional to evidence volume
  2. size        → RCT circle diameter proportional to study count
  3. color_value → bar saturation encodes confidence
  4. icon        → outcome badges with ↓/✓ symbols
  5. color_hue   → green badges (clinical) vs grey (preclinical)
  6. number      → prominent numeric callout (18, 5, 1, 40%)
  7. position_y  → top = most evidence (Stevens' power law)
  8. height      → bar height proportional (om85 tallest)

3-panel: TOP bronchus (42%) | MIDDLE evidence (25%) | BOTTOM mechanism (31%)

Reads config/layout_v22.yaml + config/palette.yaml + config/content.yaml
Produces: wireframe_GA_v22.svg + _full.png + _delivery.png
"""
...
def load_config():
    ...
    path = os.path.join(CONFIG_DIR, "layout_v22.yaml")
    with open(path, "r", encoding="utf-8") as f:
...
def draw_background(dwg, layout, palette, W, H):
    """V22: White background with subtle red→green gradient in top panel (c13 color_gradient)."""
    dwg.add(dwg.rect((0, 0), (W, H), fill="#FFFFFF"))

    # c13 channel: color_gradient — subtle left-to-right pathological(red) → protected(green)
    bg_grad = layout.get("background_gradient", {})
    if bg_grad.get("enabled", True):
        grad_id = "bg_path_to_prot"
        opacity = bg_grad.get("opacity", 0.06)
        top_h = int(H * layout.get("panels", {}).get("top", {}).get("height_pct", 0.42))

        grad = dwg.linearGradient(id=grad_id, x1="0%", y1="0%", x2="100%", y2="0%")
        grad.add_stop_color(offset="0%", color="#DC2626", opacity=opacity)
        grad.add_stop_color(offset="50%", color="#FFFFFF", opacity=0)
        grad.add_stop_color(offset="100%", color="#059669", opacity=opacity)
        dwg.defs.add(grad)
        dwg.add(dwg.rect((0, 0), (W, top_h), fill=f"url(#{grad_id})"))
...
def draw_panel_divider(dwg, layout, W, H):
    """V18: Supports multiple panel dividers."""
    # V18: panel_dividers (list) or legacy panel_divider (single)
    dividers = layout.get("panel_dividers", [])
    if not dividers:
...
def _draw_evidence_level_icon(dwg, x, y, level, size, color, font_family):
    """Draw evidence level indicator (SHAPE channel)."""
    if level == "rct_many":
        # 3 filled stars
        for i in range(3):
            sx = x + i * (size * 0.9)
            dwg.add(dwg.text("★", insert=(sx, y + size * 0.8),
                             font_size=size, fill=color, font_family=font_family))
    elif level == "rct_few":
        # 2 filled + 1 empty
        for i in range(2):
            sx = x + i * (size * 0.9)
            dwg.add(dwg.text("★", insert=(sx, y + size * 0.8),
                             font_size=size, fill=color, font_family=font_family))
        dwg.add(dwg.text("☆", insert=(x + 2 * (size * 0.9), y + size * 0.8),
                         font_size=size, fill="#D1D5DB", font_family=font_family))
    elif level == "rct_single":
        # 1 filled + 2 empty
        dwg.add(dwg.text("★", insert=(x, y + size * 0.8),
                         font_size=size, fill=color, font_family=font_family))
        for i in range(1, 3):
            sx = x + i * (size * 0.9)
            dwg.add(dwg.text("☆", insert=(sx, y + size * 0.8),
...
def draw_evidence_bars(dwg, layout, palette, W, H):
    """V18: Multi-channel evidence — 8 visual channels per claim row."""
    ev = layout["evidence"]
    font_family = layout["font"]["family"]
    ...
    if bg_color:
        bg_rx = ev.get("background_rx", 12)
        panels = layout.get("panels", {})
        ev_panel = panels.get("evidence", {})
        bg_h = int(H * ev_panel.get("height_pct", 0.25))
        bg_y = int(H * ev_panel.get("y_pct", 0.43))
        pad = ev.get("background_padding", 30)
        dwg.add(dwg.rect((ev_x - pad, bg_y), (ev_w + 2 * pad, bg_h),
    ...
    if ev.get("title_underline", False):
        ...
        dwg.add(dwg.line((ul_x1, title_y + 10), (ul_x2, title_y + 10),
    ...
    # Config sub-sections
    height_mults = ev.get("bar_height_multipliers", {})
    bar_x_offset = int(ev_w * ev.get("bar_x_offset_pct", 0.08))
    max_bar_w = int(ev_w * ev.get("max_bar_width_pct", 0.52))
    rc_cfg = ev.get("rct_circles", {})
    ob_cfg = ev.get("outcome_badges", {})
    nc_cfg = ev.get("number_callouts", {})
    el_cfg = ev.get("evidence_level", {})
    ...
    for item in ev["items"]:
        ...
        color = resolve_color(palette, f"products.{product}")
        h_mult = height_mults.get(product, 1.0)
        bar_h = int(ev["bar_height"] * h_mult)
        bar_w = int(max_bar_w * item["width_pct"])
        bar_opacity = evidence_opacity.get(product, 0.85)

        # Dynamic row height: account for multiple outcome badges
        n_outcomes = len(item.get("outcomes", []))
        badge_h = ob_cfg.get("badge_height", 44)
        badge_gap_v = ob_cfg.get("badge_gap", 6)
        badges_total_h = n_outcomes * badge_h + max(0, n_outcomes - 1) * badge_gap_v
        row_h = max(bar_h, badges_total_h)
        row_center_y = y + row_h // 2

        # ═══ CHANNEL 1: RCT count circle (SIZE) ═══
        if rc_cfg.get("enabled"):
            rct_count = item.get("rct_count", 0)
            max_rcts = rc_cfg.get("max_rcts", 18)
            max_r = rc_cfg.get("max_radius", 42)
            min_r = rc_cfg.get("min_radius", 14)

            if rct_count > 0:
                ratio = math.sqrt(rct_count / max_rcts)
                r = int(min_r + (max_r - min_r) * ratio)
            else:
                r = min_r

            circ_x = ev_x + int(ev_w * rc_cfg.get("x_pct", 0.04)) + max_r
            circ_y = row_center_y

            # Circle with product color
            dwg.add(dwg.circle(center=(circ_x, circ_y), r=r,
                               fill=color, opacity=0.92,
                               stroke=rc_cfg.get("border_color", "#FFFFFF"),
                               stroke_width=rc_cfg.get("border_width", 3)))

            # Count text inside circle
            count_text = str(rct_count) if rct_count > 0 else "—"
            count_fs = rc_cfg.get("font_size", 28) if r > 20 else 20
            dwg.add(dwg.text(count_text,
                             insert=(circ_x, circ_y + count_fs * 0.35),
                             font_size=count_fs,
                             fill=rc_cfg.get("text_color", "#FFFFFF"),
                             font_family=font_family,
                             font_weight=rc_cfg.get("font_weight", "bold"),
                             text_anchor="middle"))

        # ═══ CHANNEL 2: Evidence bar (LENGTH + COLOR_VALUE + HEIGHT) ═══
        bar_x = ev_x + bar_x_offset
        bar_y = y + (row_h - bar_h) // 2  # vertically center bar in row
        dwg.add(dwg.rect((bar_x, bar_y), (bar_w, bar_h),
                         fill=color, rx=10, ry=10, opacity=bar_opacity))

        # Accessibility pattern overlay
        pat_id = PRODUCT_PATTERN_IDS.get(product)
        if pat_id:
            dwg.add(dwg.rect((bar_x, bar_y), (bar_w, bar_h),
                             fill=f"url(#{pat_id})", rx=10, ry=10,
                             opacity=0.35))

        # Product name inside bar
        product_name = item.get("label", product.upper())
        if bar_w > 160:
            dwg.add(dwg.text(product_name,
                             insert=(bar_x + 16, bar_y + bar_h * 0.62),
                             font_size=32, fill="white",
                             font_family=font_family, font_weight="bold"))
            ev_label = item.get("evidence_label", "")
            if ev_label:
                dwg.add(dwg.text(ev_label,
                                 insert=(bar_x + bar_w - 16, bar_y + bar_h * 0.62),
                                 font_size=26, fill="white",
                                 font_family=font_family, font_weight="bold",
                                 text_anchor="end", opacity=0.90))
        elif bar_w > 80:
            dwg.add(dwg.text(product_name,
                             insert=(bar_x + 10, bar_y + bar_h * 0.62),
                             font_size=26, fill="white",
                             font_family=font_family, font_weight="bold"))
        else:
            dwg.add(dwg.text(product_name,
                             insert=(bar_x + bar_w + 10, bar_y + bar_h * 0.62),
                             font_size=24,
                             fill=palette["text"]["secondary"],
                             font_family=font_family, font_weight="bold"))

        # ═══ CHANNEL 3+4: Outcome badges (ICON + COLOR_HUE) ═══
        if ob_cfg.get("enabled"):
            badge_x = bar_x + bar_w + ob_cfg.get("x_offset", 20)
            badge_rx = ob_cfg.get("badge_rx", 22)
            badge_fs = ob_cfg.get("font_size", 24)

            outcomes = item.get("outcomes", [])
            # Center badges vertically in the row
            badges_block_h = n_outcomes * badge_h + max(0, n_outcomes - 1) * badge_gap_v
            badges_start_y = y + (row_h - badges_block_h) // 2

            for j, out in enumerate(outcomes):
                badge_y = badges_start_y + j * (badge_h + badge_gap_v)
                is_clinical = out.get("type", "clinical") == "clinical"
                bg = ob_cfg["clinical_color"] if is_clinical else ob_cfg["preclinical_color"]
                tc = ob_cfg["clinical_text"] if is_clinical else ob_cfg["preclinical_text"]

                icon = out.get("icon", "↓")
                text = out.get("text", "")
                badge_text = f"{icon} {text}"

                approx_w = len(badge_text) * badge_fs * 0.55 + 30
                approx_w = max(approx_w, 140)

                dwg.add(dwg.rect((badge_x, badge_y), (approx_w, badge_h),
                                 fill=bg, rx=badge_rx, ry=badge_rx,
                                 opacity=0.92))

                dwg.add(dwg.text(badge_text,
                                 insert=(badge_x + approx_w // 2,
                                         badge_y + badge_h * 0.65),
                                 font_size=badge_fs, fill=tc,
                                 font_family=font_family,
                                 font_weight=ob_cfg.get("font_weight", "bold"),
                                 text_anchor="middle"))

                # Horizontal connector: bar end → badge start (same y per badge)
                conn_y = badge_y + badge_h // 2
                dwg.add(dwg.line((bar_x + bar_w + 4, conn_y),
                                 (badge_x - 2, conn_y),
                                 stroke=color, stroke_width=2,
                                 opacity=0.30, stroke_dasharray="4,3"))

        # ═══ CHANNEL 5: Number callout (NUMBER) ═══
        if nc_cfg.get("enabled"):
            callout = item.get("number_callout", "")
            if callout and callout != "—":
                nc_x = ev_x + int(ev_w * nc_cfg.get("x_pct", 0.88))
                is_highlight = item.get("number_highlight", False)
                fs = nc_cfg.get("font_size_large", 56) if is_highlight else nc_cfg.get("font_size_small", 32)

                dwg.add(dwg.text(callout,
                                 insert=(nc_x, row_center_y + fs * 0.35),
                                 font_size=fs, fill=color,
                                 font_family=font_family,
                                 font_weight=str(nc_cfg.get("font_weight", 900)),
                                 text_anchor="middle",
                                 opacity=nc_cfg.get("opacity", 0.90)))

                unit = item.get("number_unit", "")
                if unit:
                    dwg.add(dwg.text(unit,
                                     insert=(nc_x, row_center_y + fs * 0.35 + 24),
                                     font_size=22,
                                     fill=palette["text"]["secondary"],
                                     font_family=font_family,
                                     text_anchor="middle",
...
def main():
    ...
    svg_path = os.path.join(OUT_DIR, "wireframe_GA_v22.svg")
    full_png = os.path.join(OUT_DIR, "wireframe_GA_v22_full.png")
    delivery_png = os.path.join(OUT_DIR, "wireframe_GA_v22_delivery.png")
