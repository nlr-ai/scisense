"""S2b scoring engine — S9a/S9b/S9c auto-scoring.
...
def score_s2b_composite(s9a: float, s9b: float, s9c: float) -> float:
    """Composite score per S2b_Mathematics.md section 7.

    Score_S2b = (0.2 * S9a) + (0.5 * S9b) + (0.3 * S9c)
    ...
    """
    # Phenomenon: S2b composite weights (S2b_Mathematics.md section 7)
...
def score_test(q1_text, q2_choice, q3_choice, correct_product, keywords=None,
    ...
    s2b = score_s2b_composite(float(s9a), float(s9b), s9c_score)
    return {
        "s9a": s9a,
        "s9a_score": s9a_score,
        "s9b": s9b,
