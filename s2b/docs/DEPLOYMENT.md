...
1. **`init_db()`** creates `data/s2b.db` with the 3-table schema if it doesn't exist. The `data/` directory is auto-created via `os.makedirs()`.
...
The SQLite database is at `data/s2b.db`.
...
```bash
sqlite3 data/s2b.db "DELETE FROM tests; DELETE FROM participants;"
```
...
4. Either delete `data/s2b.db` and restart, or manually insert into the database
...
```bash
sqlite3 data/s2b.db "SELECT id, filename, domain, version FROM ga_images;"
```
...
- Delete `data/s2b.db` and restart, or
...
### Dashboard shows wrong S2b scores for old tests

Tests created before the S2b composite and speed-accuracy columns were added may have null values. The reveal page recomputes these on the fly via `score_s9c_graduated()` and `score_s2b_composite()` when stored values are missing. The dashboard uses stored values from `get_all_tests()` -- if these are null, recomputation does not happen automatically.

Fix: delete `data/s2b.db` and re-run tests, or manually update via SQL:

```sql
UPDATE tests SET s2b_score = (0.2 * s9a_pass) + (0.5 * s9b_pass) + (0.3 * s9c_score);
```
...
| What | Path |
|------|------|
| Server | `C:\Users\reyno\scisense\s2b\app.py` |
| Database | `C:\Users\reyno\scisense\s2b\data\s2b.db` |
| Configuration | `C:\Users\reyno\scisense\s2b\config.yaml` |
| GA images (47) | `C:\Users\reyno\scisense\s2b\ga_library\` |
| Leurre images (10) | `C:\Users\reyno\scisense\s2b\ga_library\leurres\` |
| Batch generator | `C:\Users\reyno\scisense\s2b\ga_library\generate_library.py` |
| Leurre generator | `C:\Users\reyno\scisense\s2b\generate_leurres.py` |
| Embedding model cache | `~\.cache\huggingface\` |
| CSS | `C:\Users\reyno\scisense\s2b\static\style.css` |
| Templates (8) | `C:\Users\reyno\scisense\s2b\templates\` |
| Documentation | `C:\Users\reyno\scisense\s2b\docs\` |
