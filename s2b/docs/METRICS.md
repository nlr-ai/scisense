# S2b Premier Regard -- Metrics Reference

All metrics computed by the S2b platform, in one place. For mathematical derivations and justifications, see `S2b_Mathematics.md`.
...
| Property | Value |
|----------|-------|
| **Formula** | `S2b = 0.2 * S9a + 0.5 * S9b + 0.3 * S9c` |
| **Range** | [0.0, 1.0] |
| **Computed in** | `scoring.py:score_s2b_composite()` |
| **Stored in DB** | `tests.s2b_score` (real) |
| **Pass threshold** | 0.70 (config: `s2b_pass_threshold`) |
...
|---------|----------|---------------|
| Global stats | `compute_aggregate_stats()` | Taux_S9a, Taux_S9b, Score_S9c, Mediane_RT2, RT2_class, Score_S2b, N |
