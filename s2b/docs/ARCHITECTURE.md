# S2b Premier Regard -- Architecture

## Purpose

S2b is a comprehension test platform that measures whether a Graphical Abstract (GA) correctly transfers its intended information hierarchy to a viewer in 5 seconds. It is the validation layer of the Visual Evidence Compiler (VEC): the VEC generates GAs, S2b measures whether they work.
...
The core claim S2b tests: **length-encoded GAs (VEC) produce faster and more accurate hierarchy perception than area-encoded GAs (industry standard)**.

---

## Two Exposure Modes

S2b supports two test protocols on the same platform:
...
```
  ...
  Storage                            ga_library/
  =======                            ==========
  data/s2b.db (SQLite, WAL)          *.png   (47 GA images)
...
| `scoring.py` | Scoring engine. S9a stub (backward compat), S9b exact match, S9c graduated scale, S2b composite, speed-accuracy and RT2 classification. |
...
| `templates/reveal.html` | Results page. S2b composite score, S9a/S9b/S9c cards, keystroke dynamics, comparison stats. |
...
| `data/s2b.db` | SQLite database (auto-created on first run). |
...
| `docs/METRICS.md` | Comprehensive metrics reference (all S2b metrics in one place). |
...
Participant fills a form with 5 covariables: `clinical_domain`, `experience_years`, `data_literacy`, `grade_familiar`, `colorblind_status`. A UUID token is generated, stored in `participants` table, and set as an HTTP-only cookie (`s2b_token`, 30-day TTL).
...
4. **S2b composite:** `0.2*S9a + 0.5*S9b + 0.3*S9c`. See `S2b_Mathematics.md` section 7.
...
After scoring, the participant sees their results: S2b composite as a percentage with pass/fail label (threshold 0.70), individual S9a/S9b/S9c pass/fail cards, S9a semantic similarity score, keystroke dynamics for Q1 (latency classification: <1500ms immediate, 1500-4000ms standard, >4000ms reconstruction), speed-accuracy quadrant, and the original image.
...
- Global pass rates (S9a, S9b, S9c) and mean S2b composite
...
0.2*s9a + 0.5*s9b + 0.3*s9c -----> s2b_score (float)
...
**SQLite** because the expected data volume is small (hundreds to low thousands of tests). WAL mode provides concurrent read access. The database is a single file (`data/s2b.db`) that can be copied, backed up, or deleted trivially. No service to manage. No connection pooling. Foreign keys enforced via `PRAGMA foreign_keys=ON`.
...
| `s2b_score` | REAL | Composite: 0.2*S9a + 0.5*S9b + 0.3*S9c |
...
S2b connects to the VEC pipeline at these interfaces:

| Interface | Direction | Mechanism |
|-----------|-----------|-----------|
| GA images | VEC -> S2b | PNG files in `ga_library/` with sidecar JSON |
| Leurre images | Generator -> S2b | `generate_leurres.py` outputs to `ga_library/leurres/` |
| Batch generation | Generator -> S2b | `ga_library/generate_library.py` outputs PNG + JSON pairs |
| Test results | S2b -> VEC | Query SQLite `tests` table for S9b pass rates per GA version |
| A/B validation | S2b -> VEC | `compute_ab_delta()` compares control (area-encoded) vs VEC (length-encoded) |
| S10 validation | S2b -> VEC | `compute_s10_rate()` measures scroll-stopping in stream mode |
| Design iteration | S2b -> VEC | If `Taux_S9b < 0.80` for a GA version, the VEC parameters need adjustment |
| Semantic references | VEC -> S2b | GA authors write L1/L2/L3 reference texts in sidecar JSON |

S2b has no runtime dependency on VEC code. The interface is pure file-based: images and JSON in `ga_library/`.
